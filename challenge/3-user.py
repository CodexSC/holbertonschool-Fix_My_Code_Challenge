...
class User():
    ...
    def password(self, pwd):
        """Password setter — hashes in MD5 or stores None"""
        if pwd is None or type(pwd) is not str:
            self.__password = None
        else:
            self.__password = hashlib.md5(pwd.encode()).hexdigest().lower()  # fix 1: __password not _password
    def is_valid_password(self, pwd):
        """Returns True only if pwd matches the stored hash"""
        if pwd is None or type(pwd) is not str:
            return False
        if self.__password is None:
            return False
        return hashlib.md5(pwd.encode()).hexdigest().lower() == self.__password  # fix 2: .lower() not .upper()
if __name__ == '__main__':
    ...
    if not user_1.is_valid_password(u_pwd):
        print("is_valid_password should return True for right password")
    if user_1.is_valid_password("Fakepwd"):
        print("is_valid_password should return False for wrong password")
